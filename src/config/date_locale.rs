...
use crate::error::{BadDateTimeFormatterSnafu, DenimError, DenimResult, InvalidLocaleSnafu, InvalidTimezoneSnafu, RmpSerdeDecodeSnafu, RmpSerdeEncodeSnafu, S3Snafu};
...
impl TryFrom<DateLocaleConfigInterchange> for DateLocaleConfig {
    ...
    fn try_from(DateLocaleConfigInterchange { tz, locale, hour_cycle, calendar_algorithm }: DateLocaleConfigInterchange) -> Result<Self, Self::Error> {
...
impl DateLocaleConfig {
    fn dtf_prefs_and_locale_from_strings(locale: String, hour_cycle: String, calendar_algorithm: String) -> DenimResult<(Locale, DateTimeFormatterPreferences)> {
    ...
    pub fn new (timezone: String, locale: String, hour_cycle: String, calendar_algorithm: String) -> DenimResult<Self> {
        ...
        let (locale, dtf_prefs) = Self::dtf_prefs_and_locale_from_strings(locale, hour_cycle, calendar_algorithm)?;
    ...
    pub fn deserialise (bytes: impl AsRef<[u8]>) -> DenimResult<Self> {
        let interchange: DateLocaleConfigInterchange = rmp_serde::from_slice(bytes.as_ref()).context(RmpSerdeDecodeSnafu)?;
